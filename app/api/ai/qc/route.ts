...
import { formatBrandContext, generateWithGemini, parseQcResult } from "@/lib/gemini";
...
export async function POST(request: Request) {
  try {
    ...
    if (profile.role !== "owner" && !permission?.enabled) {
      throw new ApiError("QC checking is not enabled for this user.", 403);
    }
    ...
    const raw = await generateWithGemini({
      ...
      systemInstruction: settings.qc_system_prompt,
      prompt: [
        "Check this content against the brand data.",
        "Return only JSON with score, summary, issues, and recommendedFixes.",
